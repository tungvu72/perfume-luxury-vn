...
// Regex to find and replace object properties safely
for (let upd of updates) {
  let idRegex = new RegExp(`(id:\\s*"${upd.id}",[\\s\\S]*?isPublished:\\s*true\\n\\s*\\})`, 'g');
  content = content.replace(idRegex, (match) => {
    // We parse the exact string and inject/replace our new fields dynamically
    // Wait, replacing via AST or strict matched lines is better.
    let replaced = match;
    for (let key in upd.data) {
      let v = upd.data[key];
      let valStr = JSON.stringify(v);
      
      // If the field exists, replace it
      let fieldRegex = new RegExp(`\\b${key}:\\s*[\\s\\S]*?(?=,\\n|\\n\\s*\\})`);
      if (replaced.match(fieldRegex)) {
        replaced = replaced.replace(fieldRegex, `${key}: ${valStr}`);
      } else {
        // Insert after 'name' or 'description'
        replaced = replaced.replace(/(name:\s*".*?",)/, `$1\n    ${key}: ${valStr},`);
      }
    }
    return replaced;
  });
...
console.log("Successfully injected all 9 products!");
