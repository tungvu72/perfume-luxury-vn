...
updates.forEach(upd => {
  ...
  if (startIdx === -1) {
    console.error("NOT FOUND:", upd.id);
    return;
  }
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    for (let p of propsToReplace) {
      if (line.trim().startsWith(p + ':')) {
        skip = true;
        // Advanced block skipping for array properties!
        if (line.includes('[')) {
          let j = i + 1;
          // find the matching closing bracket ]
          let bracketLevel = 1;
          while (j <= endIdx && bracketLevel > 0) {
            let chLine = lines[j];
            if (chLine.includes('[')) bracketLevel++;
            if (chLine.includes(']')) bracketLevel--;
            j++;
          }
          i = j - 1; // correctly skip the whole block!
        }
        break;
      }
    }
...
console.log("Successfully patched Batch 3 with advanced block wiping technique.");
